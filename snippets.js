...
let proxyIP = 'proxy.xxxxxxxx.tk:50001';  // proxyIP:'13.230.34.30';也可以是socks5或http
let yourUUID = 'a5c0ee30-e6ba-4998-80c6-0ce6e70351d4';  // UUID
        ...
        try {
            ...
            if (pathname.startsWith('/proxyip=')) {
                ...
                if (pathProxyIP && !request.headers.get('Upgrade')) {
                    // 【修复1】：移除 proxyIP = pathProxyIP; 防止污染全局变量
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                ...
                if (url.pathname === '/') {
                    return await handleHomePage(request); // 【修复2】：增加 await
                }

                if (url.pathname === `/${yourUUID}`) {
                    return await handleSubtionPage(request); // 【修复2】：增加 await
                }

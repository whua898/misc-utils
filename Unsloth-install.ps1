...
#
# Note: Installation path is hard-coded to D:\Users\wh898\.unsloth\studio
# This avoids C: drive usage while keeping USERPROFILE on C: (where NTUSER.DAT lives)
...
function Install-UnslothStudio {
    ...
    $PythonVersion = "3.13"
    # Hard-coded installation path to D: drive to avoid C: disk usage
    # This is intentional - USERPROFILE points to C: but we install to D:
    $UserHome = "D:\Users\wh898"
    $StudioHome = Join-Path $UserHome ".unsloth\studio"
...
'@ -ErrorAction Stop
    ...
    # ── Helper: retry failed operations with exponential backoff ──
        ...
        for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {
            try {
                ...
                if ($attempt -lt $MaxRetries) {
                    ...
                    substep "$Description attempt $attempt failed, retrying in ${delay}s..." "Yellow"
                    Start-Sleep -Seconds $delay
                }
            }
        }
        
        substep "$Description failed after $MaxRetries attempts" "Red"
        return $false
    }
    ...
    function New-StudioShortcuts {
        ...
        try {
            ...
            $localAppDataDir = "D:\Users\wh898\AppData\Local"
...
try {
    ...
    try {
        `$proc = Start-Process -FilePath `$powershellExe -ArgumentList `$launchArgs -WorkingDirectory 'D:\Users\wh898' -PassThru
    } catch {
...
"@
    ...
    # ── Check winget with fallback ──
    $script:UseWinget = $true
    if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
        step "winget" "not available" "Yellow"
        substep "winget is recommended but not required" "DarkGray"
        substep "Dependencies will be installed manually or via alternative methods" "DarkGray"
        $script:UseWinget = $false
    } else {
        step "winget" (winget --version) "Green"
    }
    ...
    } elseif (Test-Path (Join-Path "D:\Users\wh898" "unsloth_studio\Scripts\python.exe")) {
        # CWD-relative venv from old install.ps1 -- migrate to absolute path
        $CwdVenv = Join-Path "D:\Users\wh898" "unsloth_studio"
    ...
    function Get-TorchIndexUrl {
        # 使用 PyTorch 官方索引 + 清华镜像作为备用源（中国国内网络优化）
    ...
    if ($_Migrated) {
        ...
        if ($SkipTorch) {
            ...
            # 添加清华镜像作为备用源，提高中国国内下载成功率
            $torchInstallExit = Invoke-InstallCommand { uv pip install --python $VenvPython "torch>=2.4,<2.11.0" torchvision torchaudio --index-url $TorchIndexUrl --extra-index-url https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple }
    ...
    # ── Check and upgrade npm if needed (before running studio setup) ──
    step "node" "checking Node.js and npm..."
    try {
        ...
        if ($NodeVersion -and $NpmVersion) {
            ...
            # Vite 8 requirements: ^20.19.0 || >=22.12.0 || >=23, npm >=11
            $NodeOk = ($NodeMajor -eq 20 -and $NodeMinor -ge 19) -or
                      ($NodeMajor -eq 22 -and $NodeMinor -ge 12) -or
                      ($NodeMajor -ge 23)
            $NpmOk = $NpmMajor -ge 11
            
            if (-not $NodeOk) {
                substep "Node $NodeVersion is too old. Required: ^20.19.0 || >=22.12.0 || >=23" "Red"
                substep "Please upgrade from https://nodejs.org/" "Yellow"
            } elseif (-not $NpmOk) {
                substep "npm $NpmVersion is too old. Required: >=11" "Yellow"
                substep "Upgrading npm automatically..." "Cyan"
                
                try {
                    npm install -g npm@latest 2>&1 | Out-Null
                    $NewNpmVersion = (npm -v 2>$null).Trim()
                    substep "npm upgraded to $NewNpmVersion" "Green"
                } catch {
                    substep "Failed to upgrade npm automatically" "Red"
                    substep "Please run manually: npm install -g npm@latest" "Yellow"
                }
            } else {
                substep "Node $NodeVersion and npm $NpmVersion meet requirements" "Green"
            }
        } else {
            substep "Node.js or npm not found" "Yellow"
            substep "Installing Node.js LTS..." "Cyan"
            
            if ($script:UseWinget) {
                try {
                    winget install -e --id OpenJS.NodeJS.LTS --accept-package-agreements --accept-source-agreements 2>&1 | Out-Null
                    Refresh-SessionPath
                    substep "Node.js installed" "Green"
                } catch {
                    substep "winget installation failed" "Red"
                }
            } else {
                substep "Download from https://nodejs.org/" "Yellow"
                substep "Or use IDM: https://nodejs.org/dist/v22.14.0/node-v22.14.0-x64.msi" "Cyan"
            }
        }
    } catch {
        substep "Could not check Node.js/npm versions" "Yellow"
    }
    ...
    # Force STUDIO_HOME and VIRTUAL_ENV to match the venv location (fixes C: vs D: drive mismatch)
    $env:STUDIO_HOME = $StudioHome
    $env:VIRTUAL_ENV = $VenvDir
    # Prepend venv Scripts to PATH so Python finds the right executables
    $env:Path = "$VenvDir\Scripts;$env:Path"
    # Temporarily override USERPROFILE to match our custom home directory
    # This ensures setup.ps1 constructs paths using the correct drive
    ...
    step "deps" "verifying Python dependencies..."
    $UvExe = Join-Path $VenvDir "Scripts\uv.exe"
    $PythonExe = Join-Path $VenvDir "Scripts\python.exe"
    
    $requiredPackages = @(
        'structlog', 'fastapi', 'starlette', 'uvicorn',
        'matplotlib', 'PyJWT', 'python-multipart', 'bcrypt',
        'httpx', 'diceware'
    )
    ...
    foreach ($pkg in $requiredPackages) {
        try {
            & $PythonExe -c "import $($pkg.Replace('-','_'))" 2>$null | Out-Null
            if ($LASTEXITCODE -ne 0) {
                $missingPackages += $pkg
            }
        } catch {
    ...
    if ($missingPackages.Count -gt 0) {
        substep "Installing missing packages: $($missingPackages -join ', ')" "Yellow"
        if (Test-Path $UvExe) {
            & $UvExe pip install $missingPackages --python $PythonExe 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) {
                substep "All missing dependencies installed" "Green"
            } else {
                substep "Failed to install some dependencies" "Red"
                substep "You may need to install them manually:" "Yellow"
                foreach ($pkg in $missingPackages) {
                    substep "  uv pip install $pkg" "DarkGray"
                }
            }
        } else {
            substep "uv not found, skipping dependency installation" "Yellow"
        }
    } else {
        substep "All dependencies verified" "Green"
    }
